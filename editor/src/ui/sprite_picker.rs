...
fn load_sprite_metadata(path: &PathBuf) -> Result<sprite_editor::SpriteMetadata, Box<dyn std::error::Error>> {
    let contents = std::fs::read_to_string(path)?;
    let metadata: sprite_editor::SpriteMetadata = serde_json::from_str(&contents)?;
    Ok(metadata)
}
